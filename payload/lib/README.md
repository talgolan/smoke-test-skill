...
| `control.zsh` | `poll_until <success-cmd> <failure-cmd> <timeout> [interval]` (returns 0=success, 2=failure signal, 1=timeout — poll BOTH signals so a fast failure aborts early); `cap <seconds> <cmd>...` (hard per-command timeout — returns the command's real rc, or 124 if killed at the cap; the per-COMMAND analogue of run.zsh's per-SECTION budget, for hang-prone daemon/service calls); `smoke_keep_on_fail` (true when section failed — DEFAULT preserves evidence; `SMOKE_KEEP_ON_FAIL=0` forces teardown — guard teardown with it); `keep_on_fail_notice <handle>...` (print live diagnostic handles). |
| `term-a.zsh` | tmux detached-session spawn / pane-grep / capture / close, plus `term_a_send` + `term_a_answer` for scripting an interactive SUT's prompts. `term_a_wait_ready "<slug>" "<probe>" [timeout]` = fail-fast readiness wait (returns 2 with the pane tail if the launch dies, instead of hanging); `term_a_launch_died "<slug>"` is its primitive. Use for any step that needs a real TTY (`docker run -it`, anything that calls `stty`) or that must answer a SUT's own setup wizard. |
