...
import { mkdtempSync, rmSync, mkdirSync, copyFileSync, chmodSync, writeFileSync } from "node:fs";
...
test("step that exceeds budget gets killed and marked TIMEOUT", () => {
  ...
  try {
    ...
    writeFileSync(stepFile, [
      ...
      "sect 'budget test'",
      "i=0",
      "while (( i < 30 )); do sleep 0.1; i=$((i+1)); done",
      "exit 0",
      "",
    ].join("\n"));
    ...
    expect(runRes.stdout).toMatch(/TIMEOUT/);
  } finally {
